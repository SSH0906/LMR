...
class SetUI(QDialog):
    ...
    def setWindow(self, title):
        ...
        self.setWindowIcon(QIcon('image/lunch_time.png'))
    ...
    def setParentVbox(self, dict_key, question_index=None):
        ...
        if dict_key == "main":
            ...
            vbox = SetUI.MWButtons(self, vbox, button_dict[dict_key])

        # classify window - question page layout 설정
            ...
            # 첫 번째 page layout 설정
            if question_index == 0:
                vbox = SetUI.CWFirstPageButtons(self, vbox, button_texts)

            # 이외 page layout 설정
            else:
                # option이 4개 이상이면 split 형태로 option 버튼 배치
                if len(button_texts) > 3:
                    vbox = SetUI.CWOptionPageSplitButtons(self, vbox, button_texts)

                # option이 4개 미만이면 stack 형태로 option 버튼 배치
                else:
                    vbox = SetUI.CWOptionPageStackButtons(self, vbox, button_texts)

                # 공통(다음) 버튼 배치
                # 마지막 질문이면 "다음" 버튼 text를 "결과 보기" 버튼으로 변경
                if question_index == len(button_dict):
                    vbox = SetUI.CWDefaultButtons(self, vbox, is_last=True)
                else:
                    vbox = SetUI.CWDefaultButtons(self, vbox, is_last=False)
        ...
        # classify window - success page layout 설정
            ...
            vbox = SetUI.CWResultPageButtons(self, vbox, button_dict[dict_key])
        ...
        # random window layout 설정
        elif dict_key == "random":
            # label에 추천 음식 추가
            random_food = _FM.FoodManagement.selectRandomFood(self)
            label = label_dict[dict_key] + f"\n[{random_food}]\n어때요?"
            vbox = SetUI.setLabel(self, label)
            vbox = SetUI.RWButtons(self, vbox, button_dict[dict_key])
    ...
    def MWButtons(self, vbox, button_texts):
    ...
    def CWFirstPageButtons(self, vbox, button_texts):
    ...
    # option이 4개 미만일 경우
    def CWOptionPageStackButtons(self, vbox, button_texts):
    ...
    # option이 4개 이상일 경우
    def CWOptionPageSplitButtons(self, vbox, button_texts):
        buttons = [MakeButton.setOptionButton(self, button_text) for button_text in button_texts]
        hbox = QHBoxLayout()
    ...
    def CWDefaultButtons(self, vbox, is_last):
        if is_last:
    ...
    # "다시 하기"(again) 버튼 배치
    def CWResultPageButtons(self, vbox, button_text):
        vbox.addWidget(MakeButton.setAgianButton(self, button_text))
        return vbox

    # random window에 버튼 배치하는 method
    # "다시 뽑기"(draw) 버튼 배치
    def RWButtons(self, vbox, button_text):
        vbox.addWidget(MakeButton.setDrawButton(self, button_text))
        return vbox
